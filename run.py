...
torch.cuda.set_device(0)  # 明确使用第一个GPU
...
# ----------------------------------------加载微调模型-------------------------------------------#
# 基础模型路径
base_model_path = "/home/student20/自然语言处理/DeepSeek-R1-Distill-Qwen-7B"
# LoRA适配器路径
lora_adapter_path = "/home/student20/自然语言处理/LLaMA-Factory/saves/DeepSeek-R1-7B-Distill/lora/train_2025-06-11-13-05-28"
...
try:
    ...
    base_model_path,
    torch_dtype=torch.float16,  # 显式指定 dtype
    local_files_only=True
).to(device)
    '''base_model = AutoModelForCausalLM.from_pretrained(
        base_model_path,
...
def main():
    # 定义测试数据集
    test_files = [
        "/home/student20/自然语言处理/test1_with_instruction.json",
